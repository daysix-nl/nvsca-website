...
add_filter( 'rest_post_collection_params', function ( $params, WP_Post_Type $post_type ) {
    if ( ( 'documenten' === $post_type->name || 'document' === $post_type->name) && isset( $params['per_page'] ) ) {
        $params['per_page']['maximum'] = 500;
        error_log('Filter for documenten is running!');
    }
...
|
*/
add_filter('rest_pre_dispatch', 'jwt_authenticate_for_rest_requestsa', 10, 3);

function jwt_authenticate_for_rest_requestsa($result, $server, $request) {
